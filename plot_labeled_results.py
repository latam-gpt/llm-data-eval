...
def plot_multilabel_results(args, data=None, dataset_name=None):
    ...
    # data cleaning
    data = {k: {key: None if val == "Sin coincidencia" else val for key, val in v.items()} for k, v in data.items()}

    # basic stats
    total_entries = len(data)
    valid_entries_count = sum(
        1 for record in data.values() if record["domains"] is not None and record["use_cases"] is not None
    )
    ...
    for record in data.values():
        ...
        if record["use_cases"]:
            use_cases.extend([use_case.strip() for use_case in record["use_cases"].split(",")])
    ...
    def sort_and_calculate_percentages(counter, top_k=None):
        ...
        sorted_items = sorted(counter.items(), key=lambda x: x[1], reverse=True) # Sort by frequency (descending)
    ...
    domain_labels, domain_values, domain_percentages = sort_and_calculate_percentages(domain_counts, top_k=10)
    ...
    plt.title(f"Top-10 Distribution of Domains for `{dataset_name}`\nExamples: {valid_entries_count:,}/{total_entries:,}")
    ...
    plt.savefig(os.path.join(args.output_dir, base_name + '_domains.png'))  # Save the figure
    
    # Sort and calculate percentages for use_cases
    use_case_labels, use_case_values, use_case_percentages = sort_and_calculate_percentages(use_case_counts, top_k=20)
    ...
    plt.title(f"Top-20 Distribution of Use Cases for `{dataset_name}`\nExamples: {valid_entries_count:,}/{total_entries:,}")
    ...
    plt.savefig(os.path.join(args.output_dir, base_name + '_usecases.png'))  # Save the figure
