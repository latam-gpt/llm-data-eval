...
def filter_by_score(
    dataset_path: str, score_threshold: int, num_proc: int, output_path: str
):
    ...
    if full_dataset is not None:
        ...
        def filter_batch(batch):
            return {
                "int_score": [s for s in batch["int_score"] if s >= score_threshold]
            }

        full_dataset = full_dataset.filter(
            filter_batch, batched=True, num_proc=num_proc
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Filter score (everything below it, gets pruned).",
    )
    parser.add_argument(
        "--num_proc",
        type=int,
        required=True,
        help="Number of CPU processes to parallelize this.",
    )
    parser.add_argument(
        "--output_path",
        type=str,
