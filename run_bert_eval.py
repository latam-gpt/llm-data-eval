...
def process_shard(file_path: str, tokenizer, model, batch_size: int, num_gpus: int, text_column: str):
    try:
...
def main(model_path: str, dataset: str, output_dir: str, num_gpus: int, batch_size: int, text_column: str):
    try:
        ...
        for file_path in tqdm(files, desc="Processing shards"):
            ...
            processed_shard = process_shard(file_path, tokenizer, model, batch_size, num_gpus, text_column)
            if processed_shard:
        ...
        if all_processed_shards:
            logging.info(f"Loading and concatenating {len(all_processed_shards)} processed shards...")
            final_dataset = concatenate_datasets([load_dataset("arrow", data_files=[path]) for path in all_processed_shards])
...
if __name__ == "__main__":
    ...
    main(args.model_path, args.dataset, args.output_dir, args.num_gpus, args.batch_size, args.text_column)